 ...
 *   dotnet CLI.
 * - It provides options via its command line, e.g.://                                  agenda
 *   - url /fl            displays list of files in server's FileStore
 *   - url /up fileSpec   uploades fileSpec to FileStore    /up ../../../sendfiles/lll.jpg
...
namespace ConsoleClient
{
  class CoreConsoleClient
  {
    ...
    private string baseUrl_;//get from cmd
    ...
    {
      MultipartFormDataContent multiContent = new MultipartFormDataContent();//multipartformcoding    std way consult google 20year

      byte[] data = File.ReadAllBytes(fileSpec);//bytearray?
      ByteArrayContent bytes = new ByteArrayContent(data);
      string fileName = Path.GetFileName(fileSpec);
      multiContent.Add(bytes, "files", fileName);

      return await client.PostAsync(baseUrl_, multiContent);//gonna return httpreponsemessage
    }
    ...
    public async Task<IEnumerable<string>> GetFileList()//
    {
      ...
      if (resp.IsSuccessStatusCode)
      {
        ...
        JArray jArr = (JArray)JsonConvert.DeserializeObject(json);//build array for each elemnt json object??
        foreach (var item in jArr)
          files.Add(item.ToString());//get key value pair
      }
      return files;
    }
    //----< download the id-th file >--------------------------

    public async Task<HttpResponseMessage> GetFile(int id)//no action just "/"
    {
    ...
    //----< Main Function pass cmd arguments >-------------------
    static void Main(string[] args)//!!!!!!!!!!!!!!
    {
      ...
      Console.Write("Press key to start: ");//server late?
      Console.ReadKey();
      ...
      switch (args[1])
      {
        case "/fl":
          ...
          break;
        case "/up":
          Task<HttpResponseMessage> tup = client.SendFile(args[2]);//http message,message dispatcher(takemessage from  display?)    //that's the client
        ...
        case "/dn":
          ...
          break;
        case "/dl":
          int id1 = Int32.Parse(args[2]);
          Task<HttpResponseMessage> tdl = client.DeleteFile(id1);
